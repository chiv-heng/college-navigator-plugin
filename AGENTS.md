...
```
college-navigator-plugin/
├── .Codex-plugin/plugin.json    # Plugin manifest (name, version, components)
...
- **Commit messages:** Include `Co-Authored-By: Codex Opus 4.6 <noreply@anthropic.com>`

## Testing

1. **Smoke test the slash command:**
   ```
   Codex --plugin-dir /path/to/college-navigator-plugin
   /college-navigator
   ```
